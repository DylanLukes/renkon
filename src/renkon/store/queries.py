...
class Queries(aiosql.queries.Queries):  # type: ignore[misc]
    ...
    This class also adds a level of safety by overriding __getattr__ to raise an error if
    the query is not defined in store.sql.
    """

    def __getattr__(self, method_name: str):
        msg = f"Query {method_name.replace('_', '-')} is not defined in store.sql"
